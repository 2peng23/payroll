...
class AttendanceController extends Controller
{
 ...
 $month_dates = [];
        for ($m=1; $m<=12; $m++) {
            $month_dates[] = date('F', mktime(0,0,0,$m, 1, date('Y')));
        }


        $attendances = Attendance::all();
        $date = request()->get('date') ?? Carbon::today(); 
        $this_month = Carbon::parse(request()->get('date'))->format('m') ?? Carbon::now()->format('m'); 
        $days = Carbon::parse($date)->month($this_month)->daysInMonth;
        $employees = Employee::where('is_active',1)->get();
        $at_days = [];
        $year = Carbon::parse(request()->get('date'))->format('Y') ?? Carbon::now()->format('Y'); 
        $month = $this_month;
       
       

        return View($this->folder.'index',compact('attendances','this_month','employees','days','year','month','month_dates'));
    }
