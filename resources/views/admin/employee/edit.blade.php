...
<div class="row">
    ...
    <div class="col-md-4">
      <div class="widget overflow-visible">
        ...
        <div class="widget-body">
              <form action="{{ route('admin.employee.deduction',$employee->id) }}" method='post'>
                @csrf
                @foreach($deductions as $dkey=> $deduct)
                 <div class="d-flexbox">
                  <div class="d-flex justify-content-between">
                    <label for="d{{ $dkey }}">
                      <input {{ \App\EmployeeDeduction::where('employee_id',$employee->id)->where('deduction_id',$deduct->id)->first() == null ? '' : 'checked' }}  id='d{{ $dkey }}' type="checkbox" name='deduction_id[]' value='{{ $deduct->id }}'> {{ $deduct->name }}</label>
                      <label> P{{ number_format( $deduct->slug == 'sss' ?  $deduct->getDeduction($employee->salary) : $deduct->amount ,2) }}</label>
                  </div>
                 </div>
                @endforeach
                 <hr>
